...
import com.ul.ts.products.mdlholder.utils.ParsingUtils;
...
public class DrivingLicence {
    ...
    private X509Certificate cscaCert;
    // checks
    ...
    private DateFormat dg1Format = new SimpleDateFormat("yyyyMMdd");

    private enum PictureType {PHOTO, SIGNATURE}

    private CSCACertificates cscaCertificates;
    ...
    public DrivingLicence(byte[] DG1, byte[] DG6, byte[] DG10,
                          ...
                          byte[] randomForAA, byte[] responseToInternalAuth/*, String mrz*/, CSCACertificates cscaCertificates) {
    ...
    private void parseDG10(byte[] DG10) {
        try (ASN1InputStream bIn = new ASN1InputStream(DG10)) {
    ...
    public void set5F06(byte[] input) {
        String timestampString = Bytes.unspacedHexString(input);
        Log.d("dateOfBirth", ParsingUtils.addDotsToDate(timestampString));
        this.dateOfBirth = ParsingUtils.addDotsToDate(timestampString);
    }
    // 5F07 placeOfBirth;
    ...
    public void set5F0A(byte[] input) {
        String timestampString = Bytes.unspacedHexString(input);
        Log.d("dateOfIssue", ParsingUtils.addDotsToDate(timestampString));
        this.dateOfIssue = ParsingUtils.addDotsToDate(timestampString);
    }
    // 5F0B dateOfExpiry;
    public void set5F0B(byte[] input) {
        String timestampString = Bytes.unspacedHexString(input);
        Log.d("dateOfExpiry", ParsingUtils.addDotsToDate(timestampString));
        this.dateOfExpiry = ParsingUtils.addDotsToDate(timestampString);
    }
...
//    }
    ...
    private void set7F63(List<byte[]> input) {
        ...
        for (byte[] b : input) {
            ...
            String fromDate = ParsingUtils.addDotsToDate(Bytes.unspacedHexString(Arrays.copyOfRange(b, parts[0].length()+1,  parts[0].length()+1+4)));
            String expiryDate = ParsingUtils.addDotsToDate(Bytes.unspacedHexString(Arrays.copyOfRange(b, parts[0].length()+1+4+1,  parts[0].length()+1+4+1+4)));
            String restrictions = "";
    ...
    private boolean verifyDSCert() {
        ...
        Principal principal = dsCert.getSubjectX500Principal();
        String subjectDn = principal.getName();
        Log.d("cert subject", subjectDn);
        principal = dsCert.getIssuerX500Principal();
        String issuerDn = principal.getName();
        Log.d("cert name", issuerDn);
        this.issuerDn = issuerDn;

        try{
            this.cscaCert = cscaCertificates.findCertificate(issuerDn.substring(3));
            Log.d("CSCA cert", "added");
        } catch (Exception e){
            Log.d("CSCA cert","Error in finding CSCA certificate");
            return false;
        }

        // Verify certificate chain
        try {
            dsCert.verify(cscaCert.getPublicKey());
        } catch (java.security.cert.CertificateException | java.security.NoSuchAlgorithmException | java.security.InvalidKeyException | java.security.NoSuchProviderException | java.security.SignatureException e) {
            Log.e(getClass().getName(), e.getMessage(), e);
            return false;
        }
        return true;
